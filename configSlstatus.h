...
static const struct arg args[] = {
        ...
        { cpu_perc,             "[%s%%]",      NULL },
        { ram_perc,             "[%s%%]",      NULL },
        { datetime,             "[%s]",         "%H:%M:%S" }, /* Date time with this format: Day name YYYY-MM-DD 18:00:00 */
};

/*

icons: ( you can find icons by typing fontawesome cheat sheet )
	sample:                     
...
static const struct arg args[] = {
	...
	{ cpu_perc, "[CPU: %s%]",      NULL },
	{ ram_perc, "[RAM: %s%]",      NULL },
	{ run_command, "[TEMP: %s]", "sensors | grep Package | awk -F' ' '{print $4}' | sed 's/+//' " },
